...
def _posicionar_nodos_optimo(nodo, pos, nivel=0, x_offset=0):
    """Posiciona nodos con distribución jerárquica óptima"""
    if nodo is None:
        return 0
    ...
    if nodo.izq and nodo.der:
        ...
        # Posicionar hijos con espacio suficiente
        x_izq = x_offset
        x_der = x_offset + ancho_izq + 4  # Mayor separación entre hijos
        
        # Recursivamente posicionar hijos
        desplazamiento_izq = _posicionar_nodos_optimo(nodo.izq, pos, nivel + 1, x_izq)
        desplazamiento_der = _posicionar_nodos_optimo(nodo.der, pos, nivel + 1, x_der)
        
        # Posicionar nodo actual en el centro
        x_centro = x_izq + (ancho_izq / 2) + ((x_der - x_izq - ancho_izq) / 2)
        pos[nodo] = (x_centro, -nivel)
        
        return ancho_izq + 4 + ancho_der
    
    elif nodo.izq:
        desplazamiento = _posicionar_nodos_optimo(nodo.izq, pos, nivel + 1, x_offset)
        pos[nodo] = (x_offset + desplazamiento / 2, -nivel)
        return desplazamiento
    
    elif nodo.der:
        desplazamiento = _posicionar_nodos_optimo(nodo.der, pos, nivel + 1, x_offset + 4)
        pos[nodo] = (x_offset + 4 + desplazamiento / 2, -nivel)
        return 4 + desplazamiento
...
def dibujar_arbol_mejorado(raiz, ax):
    ...
    pos = {}
    _posicionar_nodos_optimo(raiz, pos, 0, 0)
...
class App:
    def __init__(self, root):
        ...
        self.entry_expresion.insert(0, "A+(2-X)*Y-9/4<45^6*(9/4)>H")  # Ejemplo
